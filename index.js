...
async function run() {
    try {
        ...
        const reviewCollection = client.db('dantechDB').collection('reviews');
        const productCollection = client.db('dantechDB').collection('products');
        const userCollection = client.db('dantechDB').collection('users');
        const cartCollection = client.db('dantechDB').collection('carts');
        const wishCollection = client.db('dantechDB').collection('wishlists');
        const orderCollection = client.db('dantechDB').collection('orders');
        const productReviewCollection = client.db('dantechDB').collection('productReviews');
        const notificationCollection = client.db('dantechDB').collection('notifications');
        ...
        app.patch('/updateusers/:id', async (req, res) => {
            const id = req.params.id;  
            const { name, title, phone, address } = req.body; // Replace with fields you want to update
        
            const filter = { _id: new ObjectId(id) }; // Search by user ID
            const updatedDoc = {
                $set: {
                    name: name,
                    title: title,
...
app.get('/', (req, res) => {
    res.send('DANTECH is Running');
});

app.listen(port, () => {
    console.log(`DANTECH is Running on Port: ${port}`);
});
